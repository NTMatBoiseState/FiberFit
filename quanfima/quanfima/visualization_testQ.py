...
from quanfima.quanfima.utils2 import geo2rgb
from quanfima.quanfima import visvis_available
if visvis_available:
    import visvis as vv

def plot_orientM_map(orient_map, fiber_skel, radius_structure_elem=1,
                      figsize=(15, 15), cmap='hsv', min_label=None, max_label=None, dpi=200,
                      labelsize=20, label='Orientation, [pixels]', name=None,
                      output_dir=None):
    ...
    thickness_map = ndi.grey_dilation(orient_map, structure=disk).astype(np.float32)
    ...
    omap_ax.imshow(orient_map, cmap=cmap_obj, interpolation=None)

    display_axes = fig.add_axes([0.780, -0.076, 0.2, 0.2], projection='polar')
    display_axes_direction = np.pi
...
def plot_orientation_test_map(orient_map, fiber_skel, radius_structure_elem=1,
                         figsize=(15, 15), cmap='hsv', dpi=200, min_label='0',
    ...
    im = ax.imshow(orient_map, cmap=cmap_obj, vmin=tmin, vmax=tmax)
    divider = make_axes_locatable(ax)
    cax = divider.append_axes('right', size="2.5%", pad=0.05)
    cbar = plt.colorbar(im, cax=cax)
    cbar.ax.tick_params(labelsize=labelsize)
    ...
    if (output_dir is not None) and (name is not None):
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        fig.savefig(os.path.join(output_dir, '{}_orient_map.png'.format(name)),
                    transparent=False,
                    bbox_inches='tight',
                    pad_inches=0.1,
                    dpi=dpi)
...
def plot_diameter_map_test(thickness_map, fiber_skel, radius_structure_elem=1,
