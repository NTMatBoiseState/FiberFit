...
def process_histogram(PabsFlip, N1):
    ...
    theta1Rad=np.linspace(0.0, 2*math.pi , num = 360) #Angle array for summing (user input - 1) Divide num by angle increment input
    f1 = np.round_(N1/(2*32)) #Lower Cutoff - Frequency corresponding to 32 pixel thickness (user input - 32.0)
    f2 = np.round_(N1/(2*2)) #Upper Cutoff - Frequncy corresponding to 2 pixel thickness (user input - 2.0)

    rho1=np.linspace(f1, f2, num = (f2-f1)*2) # frequency band (sums every half step (user input - 2))
...
def process_image(name, ii):
    ...
    # fig.suptitle('%s \n Predicted Values: k = %s and th = %s degrees' %(name[:-4], krnd, thrnd))
...
def main():
    ...
    df = DataFrame({'Image Name': name, 'Kappa': k, 'Theta_p': th})
    df.to_excel('Test1.xlsx', sheet_name = 'Sheet 1', index = False)
